...
public:
	int countDigitOne(int n)
	{
		...
		if (n < 10)
			return 1;
		vector<int> countVector;  //max number of 1 in i-digits
		countVector.push_back(1);
		for (int i = 1; i < 9; i++)
			countVector.push_back(countVector.back()*10 + pow(10, i));
		int digitNum = 1;  //how many digits of n
		int mir = n;
		while (mir / 10)
		{
			digitNum++;
		...
		//...
		//my solution drive me crazy :( 
		//see the awesome solution from disscuss
	}
};
...
The following is the process to identify the relationship(function f()): The function (f(e)) is defined as the number of digit 1 increased because of digit e. (i.e. compared to there was no digit e, just (dcba), for example).
